...
<div class="relative" id="{{ $uid }}">
    <button id="{{ $uid }}-button" type="button" class="inline-flex items-center justify-center rounded-md p-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-400" aria-controls="{{ $uid }}-panel" aria-expanded="false">
        <span class="sr-only">Kosár megnyitása</span>
        <span class="relative inline-block">
            ...
            @if($count > 0)
                <span class="absolute -top-1 -right-1 inline-flex items-center justify-center rounded-full bg-rose-600 text-white text-xs px-1.5 min-w-[1.25rem] h-5 leading-none">{{ $count }}</span>
            @endif
        </span>
    </button>
    ...
    <div id="{{ $uid }}-panel" class="hidden absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-md shadow-lg z-20">
        <div class="p-3">
            @if($count === 0)
                <p class="text-slate-500">A kosár üres.</p>
            @else
                    ...
                    @foreach($items as $item)
                        ...
                        <div class="py-2 flex items-center gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="text-xs text-slate-500">{{ (int)$item->quantity }} × {{ number_format($price, 2, ',', ' ') }} {{ $currency }}</div>
                            </div>
                            <div class="text-sm font-medium whitespace-nowrap">{{ number_format($subtotal, 2, ',', ' ') }} {{ $currency }}</div>
                        </div>
                    @endforeach
                </div>
                <div class="mt-3 flex items-center justify-between">
                    <span class="text-sm text-slate-600">Végösszeg</span>
                    <span class="text-sm font-semibold">{{ number_format($total, 2, ',', ' ') }}</span>
                </div>
            @endif
            <div class="mt-3">
                <a href="{{ route('shop.cart.index') }}" class="w-full inline-flex items-center justify-center gap-2 border border-slate-300 px-3 py-2 rounded-md hover:bg-slate-50 no-underline">Kosár tartalma</a>
            </div>
        </div>
    </div>
