                ...
                @foreach($children as $child)
                    <a href="{{ route('shop.categories.show', $child) }}" class="group block rounded-xl border border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow">
                        <div class="aspect-[4/3] w-full overflow-hidden rounded-t-xl bg-slate-100">
                            @php(
                                $img = $child->image_url
                                    ?: ($child->image ? asset('storage/' . ltrim($child->image, '/')) : null)
                            )
                            @php($fallback = asset('vendor/shop/product/noimage.png'))
                            <img src="{{ $img ?: $fallback }}" alt="{{ $child->name }}" class="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" />
                        </div>
